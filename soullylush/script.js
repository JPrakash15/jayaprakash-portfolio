...
function initDraggableScreens() {
  // Touch already scrolls these natively via overflow-y:auto.
  // This adds click-and-drag panning for mouse/trackpad users.
  document.querySelectorAll('.phone-shot-scroll').forEach((scrollEl) => {
    const frame = scrollEl.closest('.phone-shot-frame');
    if (!frame) return;
    ...
    scrollEl.addEventListener('mouseleave', () => {
      // keep dragging even if the cursor briefly leaves the frame while held down
    });
  });
}
